...
import { Network, ChevronRight, Users, Sparkles } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useAppStore } from "@/lib/store";

export function NetworkWidget() {
  const [, setLocation] = useLocation();
  const { partners, networkGroups, partnerInvites } = useAppStore();

  const connected = partners.filter((p) => p.connectionStatus === "connected");
  const myNetworks = networkGroups.filter((ng) => ng.isMember || ng.isOwner);
  const pendingInvites = partnerInvites.filter((i) => i.direction === "received" && i.status === "sent");
    ...
    <div className="bg-card border border-border/40 rounded-2xl p-5 shadow-xs h-full">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground/70">Parties, networks & matchmaking</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="text-center flex-1">
          <p className="text-2xl font-semibold tabular-nums">{myNetworks.length}</p>
          <p className="text-[11px] text-muted-foreground/60">Networks</p>
        </div>
        <div className="h-8 w-px bg-border/40" />
        <div className="text-center flex-1">
          <p className="text-2xl font-semibold tabular-nums">{partners.length}</p>
          <p className="text-[11px] text-muted-foreground/60">Total parties</p>
        </div>
      </div>
      ...
      {connected.slice(0, 2).map((partner) => (
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[10px] text-muted-foreground/50">{partner.country ?? partner.region}</p>
          </div>
      ...
      {pendingInvites.length > 0 && (
        <div className="mt-2 py-2 border-t border-border/20 flex items-center gap-2">
          <Sparkles className="w-3.5 h-3.5 text-primary flex-shrink-0" />
          <p className="text-[11px] text-muted-foreground flex-1">
            {pendingInvites.length} invitation{pendingInvites.length > 1 ? "s" : ""} pending review
          </p>
        </div>
      )}
